...
description: Authors and updates customization overrides for installed Harness skills. Use when the user says 'customize harness', 'override a skill', 'change agent behavior', or 'customize a workflow'.
---

# Harness Customize

Translate the user's intent into a correctly-placed TOML override file under `{project-root}/_harness/custom/` for a customizable agent or workflow skill. Discover, route, author, write, verify.

Scope v1: per-skill `[agent]` overrides (`harness-agent-<role>.toml` / `.user.toml`) and per-skill `[workflow]` overrides (`harness-<workflow>.toml` / `.user.toml`). Central config (`{project-root}/_harness/custom/config.toml`) is out of scope — point users at the [How to Customize Harness guide](https://docs.harness-method.org/how-to/customize-harness/).

When the target's `customize.toml` doesn't expose what the user wants, say so plainly. Don't invent fields.

## Preflight

- No `{project-root}/_harness/` → Harness isn't installed. Say so, stop.
- `{project-root}/_harness/scripts/resolve_customization.py` missing → continue, but Step 6 verify falls back to manual merge.
- Both present → proceed.

## Activation

Load `_harness/config.toml` and `_harness/config.user.toml` from `{project-root}` for `user_name` (default `Harness`) and `communication_language` (default `English`). Greet. If the user's invocation already names a target skill AND a specific change, jump to Step 3.
...
- Every workflow a given agent runs → agent surface (e.g. `harness-agent-pm.toml` with `persistent_facts`, `principles`).
- One workflow only → workflow surface (e.g. `harness-prd.toml` with `activation_steps_prepend`).
...
Intent outside the exposed surface (step logic, ordering, anything not in `customize.toml`): say so; offer `activation_steps_prepend`/`append` or `persistent_facts` as approximations, or recommend `harness-builder` to create a custom skill.
...
**Template swap** (`*_template` scalar): offer to copy the default template to `{project-root}/_harness/custom/{skill-name}-{purpose}-template.md`, point the override at the new path, offer to help edit it.

## Step 5: Team or user placement

Under `{project-root}/_harness/custom/`:
...
3. Write. Create `{project-root}/_harness/custom/` if needed.
4. Verify:
   ```
   python3 {project-root}/_harness/scripts/resolve_customization.py --skill <install-path> --key <agent-or-workflow>
   ```
   Show the merged output, point out the changed fields.

   **Resolver missing or fails:** read whichever layers exist — `<install-path>/customize.toml` (base), `{project-root}/_harness/custom/{skill-name}.toml` (team), `{project-root}/_harness/custom/{skill-name}.user.toml` (user) — apply base → team → user with the same merge rules (scalars override, tables deep-merge, `code`/`id`-keyed arrays merge by key, all other arrays append), describe how the changed fields resolve.
...
- **Central config** (`{project-root}/_harness/custom/config.toml`) — see the [How to Customize Harness guide](https://docs.harness-method.org/how-to/customize-harness/).
- **Step logic, ordering, behavior not in `customize.toml`** — open a feature request, or use `harness-builder` to create a custom skill. Offer to help with either.
