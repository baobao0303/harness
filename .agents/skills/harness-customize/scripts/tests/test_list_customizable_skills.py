...
- a pre-existing team override in _harness/custom/
...
class ScannerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.custom = self.root / "_harness" / "custom"
    ...
    def test_agent_only_skill_detected(self):
        ...
        self.assertEqual(entry["name"], "harness-agent-pm")
    ...
    def test_dual_surface_skill_emits_two_entries(self):
        ...
        self.assertEqual(len(result["agents"]), 1)
        self.assertEqual(len(result["workflows"]), 1)
        self.assertEqual(result["agents"][0]["name"], "harness-dual")
        self.assertEqual(result["workflows"][0]["name"], "harness-dual")

    def test_skill_without_customize_toml_ignored(self):
        (self.skills / "harness-plain").mkdir()
        (self.skills / "harness-plain" / "SKILL.md").write_text("# plain\n")
        result = MODULE.scan_skills([self.skills], self.root)
    ...
    def test_existing_team_override_flagged(self):
        ...
        (self.custom / "harness-agent-pm.toml").write_text("[agent]\n")
        result = MODULE.scan_skills([self.skills], self.root)
        entry = result["agents"][0]
    ...
    def test_missing_surface_block_reports_error(self):
        _make_skill(self.skills, "harness-broken", "[not_a_surface]\nfoo = 1\n")
        result = MODULE.scan_skills([self.skills], self.root)
        self.assertEqual(len(result["agents"]) + len(result["workflows"]), 0)
    ...
    def test_malformed_toml_reports_error_without_aborting(self):
        skill_dir = self.skills / "harness-bad"
        skill_dir.mkdir()
        ...
        result = MODULE.scan_skills([self.skills], self.root)
        self.assertEqual(len(result["agents"]), 1)
        self.assertEqual(result["agents"][0]["name"], "harness-good")
    ...
    def test_multiple_skills_roots_are_merged(self):
        ...
        self.assertEqual(names, {"harness-agent-pm", "harness-agent-dev"})
