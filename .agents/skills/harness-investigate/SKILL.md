...
Load `{project-root}/_harness/bmm/config.yaml` and resolve `{user_name}`, `{communication_language}`,
...
Present the conclusion, then a concrete next-steps menu: trivial fix → `harness-quick-dev`; scope/plan adjustment →
`harness-correct-course`; tracked story → `harness-create-story`; fresh review → `harness-code-review`. Recommend the
