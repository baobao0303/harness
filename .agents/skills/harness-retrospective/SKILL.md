...
Run: `python3 {project-root}/_harness/scripts/resolve_customization.py --skill {skill-root} --key workflow`

**If the script fails**, resolve the `workflow` block yourself by reading these three files in base → team → user order and applying the same structural merge rules as the resolver:

1. `{skill-root}/customize.toml` — defaults
2. `{project-root}/_harness/custom/{skill-name}.toml` — team overrides
3. `{project-root}/_harness/custom/{skill-name}.user.toml` — personal overrides
...
Load config from `{project-root}/_harness/bmm/config.yaml` and resolve:
...
- `agent_roster` = resolved via `python3 {project-root}/_harness/scripts/resolve_config.py --project-root {project-root} --key agents` (merges four layers in order: `_harness/config.toml`, `_harness/config.user.toml`, `_harness/custom/config.toml`, `_harness/custom/config.user.toml`)
...
<action>Run: `python3 {project-root}/_harness/scripts/resolve_customization.py --skill {skill-root} --key workflow.on_complete` — if the resolved value is non-empty, follow it as the final terminal instruction before exiting.</action>
</step>
