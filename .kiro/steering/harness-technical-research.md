---
inclusion: manual
---

# Skill: harness-technical-research

Conduct technical research on technologies and architecture. Use when the user says they would like to do or produce a technical research report

## How to Invoke

Read and follow the full skill instructions at:
`.agents/skills/harness-technical-research/SKILL.md`

## CLI Reference

```bash
./scripts/harness skill list              # See all skills
./scripts/harness skill run harness-technical-research         # Run if wrapper exists
```
